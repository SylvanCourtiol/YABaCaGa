...
public class EditorController {
    ...
    void skillChosen(ActionEvent event) {
    	...
    	if (getDeckCost() 
    			+ (chosenSkill != null ? chosenSkill.getCost() : 0)
    			- (relevantCard.getSkill() != null ? relevantCard.getSkill().getCost() : 0) 
    			<= GameMaster.MAX_DECK_COST) {
    ...
    void update() {
        ...
        boolean deckIsValid = getDeckCost() <= GameMaster.MAX_DECK_COST && !playerName.getText().isBlank() 
        		&& !nameField.getText().isBlank() && !nameField1.getText().isBlank() && !nameField2.getText().isBlank();
